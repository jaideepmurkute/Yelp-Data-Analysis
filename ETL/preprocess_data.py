...
def remove_duplicates(df, verbose=False):
    ...
    for col in df.columns:
        if 'dict' not in str(type(df[col].values[0])):
            cols_to_consider.append(col)
    
    # Remove 'hours' column from cols_to_consider - contains None plus dict values
    if 'hours' in cols_to_consider:
        cols_to_consider.remove('hours')
    
    df.drop_duplicates(subset=cols_to_consider, inplace=True)
    if verbose: 
...
def encode_dates_fn(df, date_cols, verbose=False):
    """
...
def preprocess_data(config, df, encode_dates=False, date_cols=None, id_cols=None, 
    ...
    if config['encode_dates'] and date_cols is not None:
    ...
    col_descriptor_dict = get_column_descriptor_dict(config, df, verbose=verbose)
...
def fetch_data(config, file_name, verbose=False):
    ...
    filepath = os.path.join(config['extracted_data_dir'], file_name+'.parquet')
...
if __name__ == '__main__':
    ...
    for file_name, processing_config in data_preprocessing_config.items():
        file_name = 'yelp_academic_dataset_' + file_name
